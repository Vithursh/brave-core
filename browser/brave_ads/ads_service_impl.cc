...
constexpr int kNeedToUpdateBrowserReponseCode = 426;
...
bool AdsServiceImpl::IsNeededToUpdateBrowserToSeeAds() const {
  return need_to_update_browser_to_see_ads_;
}
...
void AdsServiceImpl::OnURLRequestComplete(
  ...
  if (response_code == kNeedToUpdateBrowserReponseCode &&
      !need_to_update_browser_to_see_ads_) {
    need_to_update_browser_to_see_ads_ = true;
    for (AdsServiceObserver& observer : observers_) {
      observer.OnNeedToUpdateBrowserToSeeAds();
    }
  }
