...
const char kNeedUpdateBrowserToSeeAds[] = "needUpdateBrowserToSeeAds";
...
base::Value BraveNewTabMessageHandler::GetAdsDataDictionary() const {
  ...
  bool need_to_update_browser_to_see_ads = false;
  if (ads_service_) {
    need_to_update_browser_to_see_ads =
        ads_service_->IsNeededToUpdateBrowserToSeeAds();
  }
  ads_data.Set(kNeedUpdateBrowserToSeeAds, need_to_update_browser_to_see_ads);
...
void BraveNewTabMessageHandler::OnNeedToUpdateBrowserToSeeAds() {
  base::Value data = GetAdsDataDictionary();
